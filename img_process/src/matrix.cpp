...
matrix::matrix(int rows, int cols, DATA_TYPE* src):
m_rows(rows), m_cols(cols){
	...
	if (NULL == mp_data || NULL == src){
		DEBUG_PRINT("matrix::matrix(int rows, int cols)\nNULL == mp_data || NULL == src \n");
	}
...
bool matrix::operator=(DATA_TYPE* src){
	if (NULL == mp_data || NULL == src){
		DEBUG_PRINT("matrix::matrix(int rows, int cols)\nNULL == mp_data || NULL == src \n");
		return false;
	}
	...
	case SHOW_IMAGE_INITAIL_VALUE:
		for (int i = SHOW_IMAGE_CROP_LENGTH; i < m_rows - SHOW_IMAGE_CROP_LENGTH; ++i){
			for (int j = SHOW_IMAGE_CROP_LENGTH; j < m_cols - SHOW_IMAGE_CROP_LENGTH; ++j){
				...
				//std::cout << std::setw(4) << mp_data[i*m_cols + j];
				//if (mp_data[i*m_cols + j] > 110){
				//	std::cout << "***";
				//}
				//else{
				//	std::cout << "   ";
